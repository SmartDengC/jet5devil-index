...
### [419. 甲板上的战舰](https://leetcode.cn/problems/battleships-in-a-board/)（20240611更新）

给你一个大小是m*n的矩阵borad表示甲板，返回甲板board上放置的战舰的数量。（把横着或者竖着x看成一艘战舰）。

战舰的个数，等于战舰头部的个数，如下图，我们只需要统计蓝色x的个数，就表示战舰的个数。

![](https://pic.leetcode.cn/1718065153-hjSHcx-lc419.png)

具体来说，如果位于(i, j)的格子是战舰的头部，那么左边和上边的相邻的格子不能是x，即：
