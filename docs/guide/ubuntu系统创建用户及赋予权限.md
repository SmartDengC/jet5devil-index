...
首先需要做的就是创建一个自己使用的角色，平常用root来没啥问题，我们还是正规一点，当做线上服务器来用，创建一个用户，那我们开始吧。
...
## 2、在ubuntu18.04中不会在创建用户的时候自动提示设置密码，需要手动执行
...
这个时候我们用`id dengc4r`就可以看到用户的信息。uid为用户id， groups表示用户组的id
...
dengc4r 为用户名， x表示密码，999表示用户id， 1000表示用户组id，`/home/dengc4r`是用户home目录， `/bin/bash`是shell命令所在的目录
