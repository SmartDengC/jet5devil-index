...
记忆化搜索是新手村神器，甚至可以用到游戏后期，推荐先看： [动态规划入门：从记忆化搜索到递推【基础算法精讲 17】](https://www.bilibili.com/video/BV1Xj411K7oF/?vd_source=35e7dde81183ac464990a0a0ab794bce)

但记忆化搜索并不是万能的，某些题目只有写成递推，才能结合数据结构来优化时间复杂度，多数题目还可以优化空间复杂度，所以尽量在写完记忆化搜索之后，把递推的代码也写一下。熟练之后直接写递推也可以。
...
##### [72. 编辑距离](https://leetcode.cn/problems/edit-distance/)（20250810）
...
求最长回文子序列，子序列元素是可以不需要连续的。
回文串就是正着念和反着念都是一样的，所以可以将字符串反转，然后求两个字符串的最长公共子序列。
还有一种就是动态规划，判断第一个字符和最后一个字符是否相同，如果相同就选择，如果不相同，就是变成了抛弃第一个字符后的字符串的最长子序列，和抛弃最后一个字符后字符串的最长子序列，这样我们就能够写出递推公式。
$$
dfs(i, j) = \begin{cases}
...
递归的出口有两个，一个是当$i > j$的时候表示没有字符，返回0， 还有就是当$i == j $的时候，表示有一个字符的情况，单个字符本身就是回文串，返回1.
...
class Solution {
    ...
    public int dfs(String s, int i, int j) {
        ...
        // same
        if (a == b) {
            return memo[i][j] = dfs(s, i + 1, j - 1) + 2;
        }
