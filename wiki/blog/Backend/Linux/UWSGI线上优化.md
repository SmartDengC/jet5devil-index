...
我们都知道 uwsgi 可以使用 `uwsgi --ini uwsgi.ini`来启动 uwsgi 服务，使用`uwsgi --stop uwsgi.pid`来停止 uwsgi 服务，使用`uwsgi --reload uwsgi.pid`来重启 uwsgi 服务。
...
1、我们需要安装，python 在 pipy.org 里面写的很清晰，直接`pip install uwsgitop`, 这里结合自己的情况，是使用 pip 还是 pip3； 也可以把安装包下载下来本地安装。
...
一般情况下，我们的 uwsgi 都是配合 nginx 使用的，所以用的都是 socket-timeout 参数。
这两者的区别简单说就是：uwsgi 单独使用就用 http, 配合 nginx 就用 socket
...
- [uwsgi 的 3 个容易掉坑的配置(timeout、harakiri、buffer-size)](https://blog.csdn.net/ch_improve/article/details/104203272)
- [uwsgi.ini 的 harakiri 配置](https://blog.csdn.net/Leon_Jinhai_Sun/article/details/133736823)
- [uwsgi 中的 max-request 是什么意思？](https://blog.csdn.net/qq_51148597/article/details/126461980)
...
昨天真的是相当惊吓的一天晚上，我在优化 uwsgi 的时候，发现重启不了了，是因为 supervisor 报错了，说没有`/tmp/supervisor.sock`这个文件，我手动 touch 一个之后，又报错说`unix:///tmp/supervisor.sock refused connection` 然后就是一直启动不起来， 我知道/tmp 下面的文件会被清掉。
...
说的是`/tmp`是大多数系统都存在的一个目录，我们可以修改成适合我们系统的一个路径；一些系统定期会删除 tmp 目录下面的旧文件，`/tmp/supervisor.sock`被删除之后 supervisorctl 将不能连接 supervisord 服务。
...
一种是把 supervisor.pid, supervisor.log 都删掉，然后在用相应的配置文件重新启动， 例如`supervisord -c ./supervisord.conf `。

另一种就是手动 touch 一个`/tmp/supervisor.sock`，然后`chmod 0700 /tmp/supervisor.sock`
...
- [Linux 命令 chmod 后为什么有时候是四位数字参数（如 0755）？【秒懂】](https://blog.csdn.net/qq_33182418/article/details/127070367)

- [解决 supervisor unix:///var/run/supervisor.sock no such file, 亲测有效](https://blog.csdn.net/qq_28119741/article/details/114122234)
...
今天的问题是 `uWSGI listen queue of scoket "0.0.0.0:8082" full (1001/1000)`

问题就是socket队列满了，导致后面的请求没办法 进来，为什么会满，因为每个uwsgi进程处理不过来，阻塞了。

![image-20250908164216445](https://gitee.com/jet5devil/typora-picture/raw/master/mac_img/202509081642923.png)

现在是加了一个参数：

 ```
 max-requests=100  # 用来解决内存问题
 harakiri = 120  # 用来解决进程阻塞问题，当请求执行超过配置的时间，会丢弃请求，直接重启这个进程
 ```
