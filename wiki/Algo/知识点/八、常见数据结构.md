...
- 区间 `[L, R]` 的异或值 = `sum[R+1] ^ sum[L]`
- 区间`[L, R)`的元素和值= `sum[R] - sum[L]`
- 区间`(L, R]`的元素和值=`sum[R+1] - sum[L+1]`
- 区间`(L, R)`的元素和值=`sum[R] - sum[L+1]`

定理：

- 如果存在两个位置 i < j 使得 sum[i] == sum[j]，那么子数组 nums[i] ~ nums[j-1] 的和为 0
