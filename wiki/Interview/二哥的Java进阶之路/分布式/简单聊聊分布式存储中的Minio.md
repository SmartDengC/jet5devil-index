...
在工作当中，遇到了需要使用存储文件的功能，昨天在面临束手无策的情况，今天下定决心认真的学习一下，后期也方便在自己的服务器中使用。

minio的安装：

面临的问题：

[minio 上传文件回显的时候403](https://blog.csdn.net/qq_39287951/article/details/131210628)



## Minio在Python中的使用

简单记录了解Minio在python中的使用。

简单列举一下minio的api

- Client.bucket_exists(bucket) 判断桶是否存在
- Client.make_bucket(bucket) 创建桶
- Client.remove_bucket(bucket) 删除桶
- client.list_buckets() 查询所有桶
- client.fput_object(bucket, file_name, file_path) 上传文件
- client.fget_object(bucket, file_name, file_path) 下载文件
- Client.remove_object(bucket, object_name) 删除桶里面的文件
...
class Bucket:
    ...
    def __init__(self, minio_address, minio_admin, minio_password):
        # 通过ip 账号 密码 连接minio server Http连接 将secure设置为False
        self.client = Minio(endpoint=minio_address, access_key=minio_admin, secret_key=minio_password, secure=False)
    ...
    def upload_file_to_bucket(self, bucket_name, file_name, file_path):
        ...
        file_name处可以创建新的目录(文件夹) 例如 /example/file_name 相当于在该桶中新建了一个example文件夹 并把文件放在其中
        :param file_path: 本地文件的路径
        """
        # 桶是否存在 不存在则新建
        check_bucket = self.client.bucket_exists(bucket_name)
        if not check_bucket:
            self.client.make_bucket(bucket_name)
        try:
            self.client.fput_object(bucket_name=bucket_name, object_name=file_name, file_path=file_path)
    ...
    def download_file_from_bucket(self, bucket_name, minio_file_path, download_file_path):
        ...
        if check_bucket:
            try:
                self.client.fget_object(bucket_name=bucket_name, object_name=minio_file_path,
    ...
    def remove_object(self, bucket_name, object_name):
        ...
        从bucket删除文件
        :param bucket_name: minio桶名称
        :param object_name: 存放在minio桶中的文件名字 object_name处可以包含目录(文件夹) 例如 /example/file_name
        """
        # 桶是否存在
        check_bucket = self.client.bucket_exists(bucket_name)
        if check_bucket:
    ...
    def get_list_objects_from_bucket_dir(self, bucket_name, dir_name):
        ...
        if check_bucket:
            ...
            objects = self.client.list_objects(bucket_name=bucket_name, prefix=dir_name, recursive=True)
            ret = []
            for _ in objects:
                ret.append(_.object_name)
            return ret
...
if __name__ == "__main__":
    ...
    bucket = Bucket(minio_address=minio_address, minio_admin=minio_admin, minio_password=minio_password)
    # 创建桶测试
