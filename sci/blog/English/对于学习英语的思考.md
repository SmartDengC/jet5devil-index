...
刚才在 github 上面看到一句话，大概意思就是能理解，但是里面有一些生词不清楚。

看能不能把这个学习生词的过程搬到现在的这个博客里面？
...
此存储库由所有者于2024年7月6日存档。它现在是只读的。 archived 把...存档；已归档的


## 20251009

Member variable visibility should be specified ；指定成员变量的可见性

Double Brace Initialization should not be used；不应使用双括号初始化，brace 支撑；支架；花括号

Mutable fields should not be "public static"；可变字段不应为“公共静态”

Math should not be performed on floats；不应在浮点数上执行数学运算

"public static" fields should be constant；“公共静态”字段应该是常量；constant 常量

Null pointers should not be dereferenced；空指针不应该被解引用， dereference 解引用，间接引用
